...
class AdminCog(CogBase):
    ...
    async def ban(
        ...
        async with self._enter_botdb_session() as (db, s):
            ...
            if await repo.is_banned_user(user.id, session=s):
                raise InvalidActionException(f"User `{user.name} ({user.id})` is already banned")
    ...
    async def unban(self, intr: Interaction[Any], user_id: str) -> None:
        ...
        async with self._enter_botdb_session() as (db, s):
            ...
            if not await repo.is_banned_user(user.id, session=s):
                raise InvalidActionException(f"User `{user.name} ({user.id})` is not banned")

            await repo.unban_user(user.id, session=s)

        await self._respond_successful(intr, f"Unbanned user `{user.name} ({user.id}).`")
    ...
    async def send(self, intr: Interaction[Any], channel_id: str, message: str) -> None:
        ...
        await self._respond_successful(
            intr,
            f"Sent message on channel `{channel.name} ({channel.id})`",  # type: ignore
        )  # type: ignore
    ...
    async def whisper(self, intr: Interaction[Any], user_id: str, message: str) -> None:
        ...
        await self._respond_successful(intr, f"Whispered message to `{user.name} ({user.id})`")
    ...
    async def whitelist(
        ...
        await self._respond_successful(intr, f"Whitelisted guild `{guild.name} ({guild.id})`")
    ...
    async def unwhitelist(self, intr: Interaction[Any], guild_id: str) -> None:
        ...
        await self._respond_successful(intr, f"Unwhitelisted guild `{guild.name} ({guild.id})`")

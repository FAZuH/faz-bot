...
class PlayerHistoryView(BaseView):
    ...
    def __init__(
        ...
        self._repo = self._bot.fazdb_db.player_history_repository
    ...
    async def _get_embed(self) -> Embed:
        ...
        player_histories = await self._repo.select_between_period(
            self._player.uuid, self._period_begin, self._period_end
        )
        player_history_earliest = player_histories[0]
        player_history_latest = player_histories[-1]
